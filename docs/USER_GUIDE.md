...
| 메시지 또는 상황                 | 해결 방법                                  |
| -------------------------------- | ------------------------------------------ |
| `저장소가 초기화되지 않았습니다.` | `INIT <user_name>` 실행                    |
| `잘못된 인자입니다.`              | 명령어의 인자 개수·옵션·따옴표 확인        |
| `존재하지 않는 브랜치: <name>`    | `BRANCH`로 생성한 브랜치명 확인            |
| `존재하지 않는 커밋: <hash>`      | `LOG` 또는 `SEARCH` 결과의 커밋 hash 사용  |
| `경로가 없습니다.`                | 두 커밋이 같은 연결 그래프에 속하는지 확인 |
| `파일 오류: ...`                  | 파일 경로·읽기 권한·UTF-8 인코딩 확인      |
| `벤치마크 검증에 실패했습니다.`    | 동일 입력의 두 정렬 결과 불일치 여부 확인  |
